...
int main() {
	...
	oa << boost::serialization::make_nvp("RFC", rf );
	//the serialized RFC is stored in oss.str() string
	//uncomment if You want to see the xml
	//cout << oss.str();

	RFC emptyRf( attribs, cat ); //crete the 'empty' classifier
	string ET[] = { "overcast", "hot", "high", "weak"};
	ExampleTest et = createExample( ET, ET + 4, rf);

	std::istringstream iss(oss.str());
	boost::archive::xml_iarchive ia(iss);
	ia >> boost::serialization::make_nvp("RFC", emptyRf); //de-serialize the stored object
	ExampleTest et2 = createExample( ET, ET + 4, emptyRf);

	//both classifiers should return the same classes
	std::cout << rf.getCategories(et) << std::endl;
	std::cout << "===DESERIALIZED:===" << std::endl;
	std::cout << emptyRf.getCategories(et2) << std::endl;
